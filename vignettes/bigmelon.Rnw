...
to your data, such as those previously described by Pidsley, Wong et al. [1] 
...
and are split into two files per sample (one for the Red Channel and 
one Green Channel). Typically these are read into R using \Rpackage{minfi} or 
...
\Rfunction{methylumIDATepic} (\Rfunction{wateRmelon} to read in the data - 
although it should be noted that the full annotation of features will 
not be included.

\Rfunction{iadd} and \Rfunction{iadd2} functions vary slightly differently
as \Rfunction{iadd} will take a vector of barcodes, \Rfunction{iadd2} will 
accepta directory pathway and extract all IDAT files within the specified path.

\Rfunction{iadd2} additionally has the functionality to read IDAT files in 
chunks if you are attempting to read in a lot of data at once and do not have 
sufficient memory on your workstation to support this. 
...
or \Rclass{MethylSet} you can pass the object to \Rfunction{es2gds} to 
convert the data into a \Rclass{gds.class}
...
there are nodes with the \Rclass{gdsn.class} class. To access these, 
...
Typically the \Rpackage{bigmelon} gds file is comprised on some common nodes 
...
Majority of these accessors will make call to \Rfunction{index.gdsn} but if 
...
object represented in a gdsn.class object. 
...
\Rfunction{read.gdsn} (and subsets using \Rfunction{readex.gdsn})
...
Additionally it is possible to call a gds node from a gds file within the 
\Rfunction{'['} indexing.
<<code-block, keep.source=TRUE>>=
...
Within \Rpackage{bigmelon} there are numerous normalization methods that can 
...
replace raw intensities with the normalized ones. This will not work if gds 
...
Due to how the normalization process is broken down within \Rpackage{bigmelon} 
...
features), totalling around 28Gb of data. Simple quantile normalization 
